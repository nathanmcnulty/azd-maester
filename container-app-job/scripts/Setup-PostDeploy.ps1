...
$jobsResponse = Invoke-AzRestMethod -Method GET -Path $jobsQuery
...
Write-Host "Container App Job '$containerAppJobName' managed identity is configured for Maester Graph permissions."
...
if ($IncludeExchange -and $exchangeSetupStatus -eq 'pending') {
  ...
  try {
    ...
    if ($newAssignmentId) {
      ...
      Write-Host 'Assigned Exchange.ManageAsApp to the Container App Job managed identity.'
    }
    else {
      Write-Host 'Exchange.ManageAsApp is already assigned to the Container App Job managed identity.'
    }
...
if ($IncludeTeams -and $teamsSetupStatus -eq 'pending') {
  try {
    ...
    if ($newTeamsRoleAssignmentId) {
      ...
      Write-Host "Entra directory role 'Teams Reader' is already assigned to the Container App Job managed identity."
    }
...
$keysResponse = Invoke-AzRestMethod -Method POST -Path $storageKeyPath
if ($keysResponse.StatusCode -ne 200) {
  throw "Failed to retrieve storage account keys. HTTP $($keysResponse.StatusCode)"
}
