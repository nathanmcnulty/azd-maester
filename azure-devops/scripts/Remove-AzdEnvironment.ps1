...
function Remove-ResourceGroupLocks {
  ...
  $subscriptionParam = @('--subscription', $SubscriptionId)
...
$adoProject = $null
$adoRepositoryName = $null
$adoRepositoryId = $null
$adoPipelineName = $null
$adoPipelineId = $null
$adoServiceConnectionName = $null
$adoServiceConnectionId = $null
$workloadAppId = $null
$workloadAppObjectId = $null
$workloadServicePrincipalObjectId = $null
...
$baseRoleAssignmentIdsJson = $null
...
$workloadIdentityDisplayName = $null
...
if ($LASTEXITCODE -eq 0) {
  ...
  $tenantId = Get-EnvValue -Lines $envValues -Name 'AZURE_TENANT_ID'
  $adoOrganization = Get-EnvValue -Lines $envValues -Name 'AZDO_ORGANIZATION'
  $adoProject = Get-EnvValue -Lines $envValues -Name 'AZDO_PROJECT'
  $adoRepositoryName = Get-EnvValue -Lines $envValues -Name 'AZDO_REPOSITORY'
  $adoRepositoryId = Get-EnvValue -Lines $envValues -Name 'AZDO_REPOSITORY_ID'
  $adoPipelineName = Get-EnvValue -Lines $envValues -Name 'AZDO_PIPELINE_NAME'
  $adoPipelineId = Get-EnvValue -Lines $envValues -Name 'AZDO_PIPELINE_ID'
  $adoServiceConnectionName = Get-EnvValue -Lines $envValues -Name 'AZDO_SERVICE_CONNECTION_NAME'
  $adoServiceConnectionId = Get-EnvValue -Lines $envValues -Name 'AZDO_SERVICE_CONNECTION_ID'
  $workloadAppId = Get-EnvValue -Lines $envValues -Name 'AZDO_WORKLOAD_APP_ID'
  $workloadAppObjectId = Get-EnvValue -Lines $envValues -Name 'AZDO_WORKLOAD_APP_OBJECT_ID'
  $workloadServicePrincipalObjectId = Get-EnvValue -Lines $envValues -Name 'AZDO_WORKLOAD_SERVICE_PRINCIPAL_OBJECT_ID'
  ...
  $baseRoleAssignmentIdsJson = Get-EnvValue -Lines $envValues -Name 'AZDO_BASE_ROLE_ASSIGNMENT_IDS'
  ...
  $workloadIdentityDisplayName = Get-EnvValue -Lines $envValues -Name 'AZDO_WORKLOAD_IDENTITY_DISPLAY_NAME'
}

$baseRoleAssignmentIds = ConvertFrom-JsonArrayOrEmpty -Json $baseRoleAssignmentIdsJson
...
if (-not [string]::IsNullOrWhiteSpace($workloadServicePrincipalObjectId) -and @($exoAppRoleAssignmentIds).Count -gt 0) {
  ...
  foreach ($assignmentId in @($exoAppRoleAssignmentIds)) {
    ...
    & az rest --method delete --url "https://graph.microsoft.com/v1.0/servicePrincipals/$workloadServicePrincipalObjectId/appRoleAssignments/$assignmentId" | Out-Null
    if ($LASTEXITCODE -ne 0) {
...
if ($exchangeIdentityCandidates.Count -gt 0) {
  Write-Host "Attempting Exchange RBAC cleanup for candidate identities: $($exchangeIdentityCandidates -join ', ') (best-effort)..."
  try {
    if (Test-ModuleAvailable -ModuleName 'ExchangeOnlineManagement') {
      $exoToken = $null
      $exoOrganization = $null
      try {
        $exoTokenResponse = Get-AzAccessToken -ResourceUrl 'https://outlook.office365.com' -AsSecureString
        $exoToken = ConvertTo-PlainTextToken -TokenValue $exoTokenResponse.Token
        if ($exoToken) {
      ...
      foreach ($identity in $exchangeIdentityCandidates) {
        try {
          $assignments = Get-ManagementRoleAssignment -RoleAssignee $identity -ErrorAction SilentlyContinue
          ...
          Write-Warning ("Failed to remove Exchange management role assignments for '{0}'. Error: {1}" -f $identity, $_.Exception.Message)
        }
      }

      foreach ($identity in $exchangeIdentityCandidates) {
        try {
          Remove-ServicePrincipal -Identity $identity -Confirm:$false -ErrorAction SilentlyContinue
        }
        catch {
        }
      }
...
if ($KeepEnvironment) {
  Write-Host 'Clearing Azure DevOps and Easy Auth metadata from kept azd environment...'
  $namesToClear = @(
    'EASY_AUTH_ENTRA_APP_OBJECT_ID',
    'EASY_AUTH_ENTRA_APP_CLIENT_ID',
    'EASY_AUTH_ENTRA_APP_DISPLAY_NAME',
    'AZDO_ORGANIZATION',
    'AZDO_PROJECT',
    'AZDO_REPOSITORY',
    'AZDO_REPOSITORY_ID',
    ...
    'AZDO_WORKLOAD_APP_ID',
    'AZDO_WORKLOAD_APP_OBJECT_ID',
    'AZDO_WORKLOAD_SERVICE_PRINCIPAL_OBJECT_ID',
    'AZDO_WORKLOAD_IDENTITY_DISPLAY_NAME',
    'AZDO_LAST_PIPELINE_RUN_ID',
    'AZDO_LAST_PIPELINE_RUN_URL',
    'AZDO_PIPELINE_FILES_PUSHED',
    'AZDO_MANUAL_PIPELINE_FILES_PATH',
    'AZDO_CREATE_REPO_IF_MISSING',
    'AZDO_PUSH_PIPELINE_FILES',
    'AZDO_VALIDATE_PIPELINE_RUN',
    'AZDO_PIPELINE_YAML_PATH',
    'AZDO_DEFAULT_BRANCH',
    'AZDO_SCHEDULE_CRON',
    'AZDO_BASE_ROLE_ASSIGNMENT_IDS',
    'INCLUDE_EXCHANGE',
    'INCLUDE_TEAMS',
    'INCLUDE_AZURE',
    'AZURE_RBAC_SCOPES',
    'SETUP_EXCHANGE_STATUS',
    'SETUP_TEAMS_STATUS',
    'SETUP_AZURE_STATUS',
    'EXO_APPROLE_ASSIGNMENT_IDS',
    'TEAMS_READER_ROLE_ASSIGNMENT_IDS',
    'AZURE_ROLE_ASSIGNMENT_IDS',
    'EXO_SERVICE_PRINCIPAL_DISPLAY_NAME'
  )

  foreach ($name in $namesToClear) {
    Clear-AzdEnvironmentValue -Name $name
  }
}
